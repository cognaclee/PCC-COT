...
def compress(args, model, xyzs, feats, save_dir, name):
    # input: (b, c, n)
    ...
    '''
    for i in range(1,len(gt_xyzs)):
        save_xyz = gt_xyzs[i].detach().cpu().squeeze(0).transpose(1,0).numpy()
        save_pcd(save_dir, name + '_'+str(i) + '.ply', gt_xyzs[i].detach().cpu().squeeze(0).transpose(1,0).numpy())
    
    save_pcd(save_dir, name + '_'+str(len(gt_xyzs)) + '.ply', latent_xyzs.detach().cpu().squeeze(0).transpose(1,0).numpy())
    '''
...
def test_xyzs(args):
    ...
    with torch.no_grad():
        for i, input_dict in enumerate(test_loader):
            ...
            actual_bpp = compress(args, model, xyzs, feats, pred_patch_dir, str(i))
            ...
            # save patches
            baseName = str(pcd_num) + '_'+ str(i) + '.ply'
            save_pcd(gt_patch_dir, baseName, original_gt_patches)
            save_pcd(pred_patch_dir, baseName, original_pred_patches)
...
def test_normals(args):
    ...
    with torch.no_grad():
        for i, input_dict in enumerate(test_loader):
            ...
            baseName = str(pcd_num) + '_'+ str(i) + '.ply'
            save_pcd(gt_patch_dir, baseName, original_gt_patches, gt_normals)
            save_pcd(pred_patch_dir,baseName, original_pred_patches, pred_normals)
...
def parse_test_args():
    ...
    parser.add_argument('--model_path', default='./output/2022/ckpt/ckpt-best.pth', type=str, help='path to ckpt')
